...
relabeled) so a re-extraction is mechanical. Pre-existing content outside the gate's paths — the curated
rating field with values strong/ok/trap and its viewer control, the first
paragraph of the README, two scripts' User-Agent strings, and several curated
notes — predates this pass and was left untouched; ratify whether to rename
the field, generalize the User-Agent strings, and rewrite those notes, and
whether the gate should then cover `data/curated.json`, `README.md` and
`scripts/` as well.
